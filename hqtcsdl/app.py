...
def delete_article(article_id):
    ...
    try:
        ...
        cursor.execute("DELETE FROM BINHLUAN WHERE MaBaiViet = ?", (article_id,))

        # Xóa bài viết
        cursor.execute("DELETE FROM BAIVIET WHERE MaBaiViet = ?", (article_id,))

        conn.commit()
        flash("Bài viết và các bình luận liên quan đã được xóa thành công!")
    except Exception as e:
        conn.rollback()
        flash(f"Lỗi khi xóa bài viết: {e}")
    finally:
        conn.close()
